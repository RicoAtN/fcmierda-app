...
function normalizeNew(payload: any) {
  ...
  out.competition_champion =
    typeof out.competition_champion === "string" && out.competition_champion.trim().length
      ...
      : null;
  out.league_link =
    typeof out.league_link === "string" && out.league_link.trim().length
      ? out.league_link.trim()
      : null;
  ...
  try {
    ...
    if (orgCol === "organisation") {
      if (schema.opponentsType === "jsonb") {
        await sql`
          INSERT INTO competition (organisation, division, competition_name, total_teams, start_period, end_period, football_type, fcmierda_final_rank, competition_champion, opponents, league_link)
          VALUES (${payload.organisation}, ${payload.division}, ${payload.competition_name}, ${payload.total_teams}, ${payload.start_period}, ${payload.end_period}, ${payload.football_type}, ${payload.fcmierda_final_rank}, ${payload.competition_champion}, ${JSON.stringify(opponentsArr)}::jsonb, ${payload.league_link})
        `;
      } else if (schema.opponentsType === "text[]") {
        await sql`
          INSERT INTO competition (organisation, division, competition_name, total_teams, start_period, end_period, football_type, fcmierda_final_rank, competition_champion, opponents, league_link)
          VALUES (${payload.organisation}, ${payload.division}, ${payload.competition_name}, ${payload.total_teams}, ${payload.start_period}, ${payload.end_period}, ${payload.football_type}, ${payload.fcmierda_final_rank}, ${payload.competition_champion}, ${arrayLiteral}::text[], ${payload.league_link})
        `;
      } else {
        await sql`
          INSERT INTO competition (organisation, division, competition_name, total_teams, start_period, end_period, football_type, fcmierda_final_rank, competition_champion, opponents, league_link)
          VALUES (${payload.organisation}, ${payload.division}, ${payload.competition_name}, ${payload.total_teams}, ${payload.start_period}, ${payload.end_period}, ${payload.football_type}, ${payload.fcmierda_final_rank}, ${payload.competition_champion}, ${JSON.stringify(opponentsArr)}, ${payload.league_link})
        `;
      }
    } else if (orgCol === "organization") {
      if (schema.opponentsType === "jsonb") {
        await sql`
          INSERT INTO competition (organization, division, competition_name, total_teams, start_period, end_period, football_type, fcmierda_final_rank, competition_champion, opponents, league_link)
          VALUES (${payload.organisation}, ${payload.division}, ${payload.competition_name}, ${payload.total_teams}, ${payload.start_period}, ${payload.end_period}, ${payload.football_type}, ${payload.fcmierda_final_rank}, ${payload.competition_champion}, ${JSON.stringify(opponentsArr)}::jsonb, ${payload.league_link})
        `;
      } else if (schema.opponentsType === "text[]") {
        await sql`
      ...
      if (schema.opponentsType === "jsonb") {
        await sql`
          INSERT INTO competition (division, competition_name, total_teams, start_period, end_period, football_type, fcmierda_final_rank, competition_champion, opponents, league_link)
          VALUES (${payload.division}, ${payload.competition_name}, ${payload.total_teams}, ${payload.start_period}, ${payload.end_period}, ${payload.football_type}, ${payload.fcmierda_final_rank}, ${payload.competition_champion}, ${JSON.stringify(opponentsArr)}::jsonb, ${payload.league_link})
        `;
      } else if (schema.opponentsType === "text[]") {
        await sql`
          INSERT INTO competition (division, competition_name, total_teams, start_period, end_period, football_type, fcmierda_final_rank, competition_champion, opponents, league_link)
          VALUES (${payload.division}, ${payload.competition_name}, ${payload.total_teams}, ${payload.start_period}, ${payload.end_period}, ${payload.football_type}, ${payload.fcmierda_final_rank}, ${payload.competition_champion}, ${arrayLiteral}::text[], ${payload.league_link})
        `;
      } else {
        await sql`
          INSERT INTO competition (division, competition_name, total_teams, start_period, end_period, football_type, fcmierda_final_rank, competition_champion, opponents, league_link)
          VALUES (${payload.division}, ${payload.competition_name}, ${payload.total_teams}, ${payload.start_period}, ${payload.end_period}, ${payload.football_type}, ${payload.fcmierda_final_rank}, ${payload.competition_champion}, ${JSON.stringify(opponentsArr)}, ${payload.league_link})
        `;
      }
    }
