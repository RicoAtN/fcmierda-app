...
const compareLocalPlayers = (a: Player, b: Player) => {
  const ap = hasPhoto(a), bp = hasPhoto(b);
  if (ap !== bp) return ap ? -1 : 1;

  const ar = roleRank(a.role), br = roleRank(b.role);
  if (ar !== br) return ar - br;

  const na = toNum(a.number), nb = toNum(b.number);
...
export default function TeamPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.sort(compareLocalPlayers);
  }, [query, roleFilter]);
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Left: Controls + list for Meet the Team (local) */}
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="text-right">
                        <div className="text-2xl sm:text-3xl font-extrabold text-green-300">{displayNumber(selected.number)}</div>
                      </div>
                    </div>
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="mt-6">
                          ...
                          <div className="text-xs text-gray-400 mb-1">Squad number: {displayNumber(selected.number)}</div>
        ...
        <header id="meet-team-2" className="mt-12 mb-6 text-center">
          <h1 className={`text-3xl sm:text-4xl font-extrabold ${robotoSlab.className}`}>Meet the Team 2</h1>
          <p className={`mt-2 text-sm sm:text-base text-gray-300 ${montserrat.className}`}>
Detailed player profiles with roles, bios, call signs and current performance statistics.          </p>
        </header>
